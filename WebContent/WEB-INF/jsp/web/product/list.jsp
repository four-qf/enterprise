    ...
    <div class="left-nav">
      <ul>
        <c:forEach items="${productkinds }" var="productkind" >
          <li><a href="/enterprise/web/product/list/${productkind.productkindid}">${productkind.productkindname }</a></li>
        </c:forEach>  
      </ul>
    </div>
    <div class="phone-bj">
    ...
    <div class="Pro_Div">
        <ul>
         <c:forEach items="${productlist }" var="product" begin="${(pageNow-1)*pageSize }" end="${pageNow*pageSize -1 }">
          <li>
            <div class="proju">
              <a class="a_img" href="/enterprise/web/product/showproduct/${product.productid}" title="#" hidefocus="true">
                   <img class="img" src="/enterprise/uploadFiles/${product.imagesnewname}" width="170" height="115" alt="#">
              </a>
              <span>
                <a class="privilege" href="/enterprise/web/product/showproduct/${product.productid}" title="${product.productname }">${product.productname }</a>
              </span>
       ...
       <div style="clear:botn"></div><br/>
        </div>
     </div>  
       <div class="page" style="margin-left: 300px">
          <span>共<b>${pageCount }</b>页</span>
        <c:if test="${pageNow>1 }">
        	<a href='/enterprise/web/product/getPageNow/${pageNow -1 }'><img src='/enterprise/images/back.gif' width='34' height='13' /></a>
        </c:if>
		<a >${pageNow }</a>
		<c:if test="${pageNow <pageCount }">
		<a href='/enterprise/web/product/getPageNow/${pageNow +1 }'><img src='/enterprise/images/next.gif' width='34' height='13' /></a>
		</c:if>
		</div>
